...
const coustmerModel = require("../models/costumermodel")
...
const createOrder= async function (req, res) {
    ...
    let count_Balance = await coustmerModel.findById({_id:coustmer}).select({balance:1})
    let product_Price = await productModel.findById({_id:Product}).select({price:1})
   
     let a = count_Balance.balance
